...
function scoreWord(wt, word, isMain, extraChips) {
  ...
  var fm = (wm + em) * xm; var total = Math.round(letters * fm);
...
function scoreWordDetailed(wt, word, isMain, extraChips) {
  ...
  if (isMain) {
    ...
    if (st.bhXm && st.bhXm > 1) {
      gevSpecial.push({
        type: 'global', icon: 'BH', fg: '#c0e080', name: 'Bounty Hunter',
...
async function runScoreAnim(events, crossLetters, grandTotal) {
  ...
  var saL = document.getElementById('ls-chips'), saM = document.getElementById('ls-mult'), saS = document.getElementById('ls-score');
  ...
  for (var i = 0; i < events.length; i++) {
    ...
    if (ev.type === 'tile') {
      ...
      for (var trig = 0; trig < ev.triggers; trig++) {
        ...
        saL.textContent = letters; saS.textContent = Math.round(letters * mult).toLocaleString(); bumpSA('ls-chips');
        if (trig === 0) {
        ...
        if (trig === 1 && ev.sqRetrigger && r) { showScorePop(ev.sqEff.label + '!', r.left + r.width / 2 - 28, r.top - 30, '#500808', '#ff8080'); mult = ev.wm + ev.em; saM.textContent = Math.round(mult * 10) / 10; bumpSA('ls-mult'); }
      ...
      if (ev.lettersDelta) { letters += ev.lettersDelta; showScorePop(ev.lettersLabel || '+' + ev.lettersDelta, br.left + 24, br.top - 32, '#0d2a50', '#80c8ff'); saL.textContent = letters; bumpSA('ls-chips'); }
  ...
  if (crossLetters > 0) {
    ...
    var curSaS = parseInt(saS.textContent.replace(/,/g, '')) || 0;
    saS.textContent = (curSaS + crossLetters).toLocaleString();
    bumpSA('ls-chips'); await scoreDelay(Math.max(120, Math.max(minDelay, delay) * 1.5));
  }
...
function updateLiveScore() {
  var lsC = document.getElementById('ls-chips'), lsM = document.getElementById('ls-mult'), lsS = document.getElementById('ls-score');
  ...
  lsM.textContent = Math.round(res.mult * 10) / 10;
